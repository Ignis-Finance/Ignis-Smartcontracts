...
/**
 * @title Ignis's IgnisErc20Delegator Contract
 * @notice IgnisTokens which wrap an EIP-20 underlying and delegate to an implementation
 */
contract IgnisErc20Delegator is IgnisTokenInterface, IgnisErc20Interface, IgnisDelegatorInterface {
    /**
    ...
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData) override public {
        require(msg.sender == admin, "IgnisErc20Delegator::_setImplementation: Caller must be admin");
    ...
    function liquidateBorrow(address borrower, uint repayAmount, IgnisTokenInterface ignisTokenCollateral) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, ignisTokenCollateral));
        return abi.decode(data, (uint));
    }
   ...
   /**
    ...
    receive() external payable {
        require(msg.value == 0,"IgnisErc20Delegator:fallback: cannot send value to fallback");

        // delegate all other functions to current implementation
        bool success = _payableSubFunc();
    ...
    function _payableSubFunc() private returns(bool) {
      (bool success, ) = implementation.delegatecall(msg.data);
      return success;
    }
}
