...
contract ComptrollerVXStorage is UnitrollerAdminStorage {
     ...
     */
    mapping(address => IgnisToken[]) public accountAssets;
    ...
    struct Market {
        ...
        /// @notice Whether or not this market receives IGNIS
        bool isIgnised;
    }

    /**
     * @notice Official mapping of IgnisTokens -> Market metadata
    ...
    IgnisToken[] public allMarkets;

    // @notice The borrowCapGuardian can set borrowCaps to any number for any market. Lowering the borrow cap could disable borrowing on the given market.
    address public borrowCapGuardian;

    // @notice Borrow caps enforced by borrowAllowed for each ignisToken address. Defaults to zero which corresponds to unlimited borrowing.
    ...
    /// @notice The IGNIS/FLR market supply state for each market
    mapping(uint8 => mapping(address => RewardMarketState)) public rewardSupplyState;

    /// @notice The IGNIS/FLR market borrow state for each market
    mapping(uint8 =>mapping(address => RewardMarketState)) public rewardBorrowState;

    /// @notice The IGNIS/FLR borrow index for each market for each supplier as of the last time they accrued reward
    mapping(uint8 => mapping(address => mapping(address => uint))) public rewardSupplierIndex;

    /// @notice The IGNIS/FLR borrow index for each market for each borrower as of the last time they accrued reward
    mapping(uint8 => mapping(address => mapping(address => uint))) public rewardBorrowerIndex;

    /// @notice The IGNIS/FLR accrued but not yet transferred to each user
    mapping(uint8 => mapping(address => uint)) public rewardAccrued;

    /// @notice IGNIS token contract address
    address public ignisAddress;
